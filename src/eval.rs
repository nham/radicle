...
fn eval_atom(env: Env, vec: Vec<Expr>) -> EvalResult {
    if vec.len() != 2 {
        ...
        let val = try!( eval(env.clone(), *vec.get(1)) ).val1();
...
fn eval_eq(env: Env, vec: Vec<Expr>) -> EvalResult {
    ...
    if vec.len() != 3 {
        ...
        let val2 = try!( eval(env.clone(), *vec.get(2)) ).val1();
...
fn eval_defun(env: Env, vec: Vec<Expr>) -> EvalResult {
    if vec.len() != 4 {
        ...
        new_env.bindings.insert(vec.get(1).unwrap_leaf(), label_expr);
        Ok( (new_env, Nil) )
    }
}
